...
A superfície conversacional do kernel em `metodo/`. O app `docs/index.html` é o mesmo método em formulário, sem IA embutida (na Etapa 2 ele tem um *modo assistido* por copiar-e-colar: gera o prompt, lê a resposta colada e deixa a pessoa aceitar candidato a candidato — mesma regra de `origem` desta skill); aqui a IA ocupa o passo 1 do fluxo de validação da Aula 2 — **IA gera recomendação → humano valida e decide**. O registro produzido tem os mesmos campos e os mesmos IDs do app, porque os dois leem o mesmo kernel.
